...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    with conn:
        ...
        while True:
            ...
            dX = puckCoordX - oldPuckCoorX
            dY = puckCoordY - oldPuckCoorY
            ...
            oldPuckCoorX = puckCoordX
            oldPuckCoorY = puckCoordY

            velocityX = dX / dt
            velocityY = dY / dt

            target_x_new = puckCoordX + velocityX * 0.3 
            if abs(velocityY) > 1: #absolute value
                max_defense_position
            else:
                target_y_new = puckCoordY + velocityY * 0.3
            #target_y_new = puckCoordY + velocityY * 0.5 #can do algorithm where given seconds changes depending on dt/ how close it is

            #when puck bounces off wall: #check border values ???
            if ((puckCoordX <= 0) | (puckCoordX >= 475)):
                velocityX = -velocityX
            if puckCoordY <= 0: 
                velocityY = -velocityY
            ...
            conn.sendall(data)
            #ser.write(data)
            #print("x: {}, y: {}, s: {}, a: {}, rx: {}, ry: {}".format(target_x_new, target_y_new, max_speed, max_accel, robotCoordX, robotCoordY))mjn  